...
func TcpForward(src, dst net.Conn) {
	...
	// dstConn.SetKeepAlivePeriod(time.Second * 5) // 链接保持时间，默认15s
	// dstConn.SetDeadline(time.Now().Add(time.Second * 60)) // 读写数据超时时间
	...
	// srConn.SetKeepAlivePeriod(time.Second * 5) // 链接保持时间
